...
export async function deleteEntity(
  ...
  if (mode === 'delete' && impact.requiresAdmin) {
    return {
      ...
      requiresAdmin: true,
      message:
        'לרשומה זו קיימת היסטוריה חתומה ולכן לא ניתן למחוק אותה באופן רגיל. ניתן להסיר אותה מהרשימות הפעילות תוך שמירת המסמכים והיסטוריית החתימות, או לבקש אישור מנהל מערכת.',
    }
  }
