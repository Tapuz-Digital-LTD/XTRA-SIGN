...
const registration = (token: string | undefined) => ({
  ...
  values: { businessName: 'עסק CAPTCHA', taxId: `51${String(Date.now()).slice(-7)}`, signatoryName: 'ישראל ישראלי', signatoryRole: 'מנהל', benefit1: '25% על כל ההזמנה', week: 'week_1', redemption: 'generic_xtra25', declareLicense: true, declareInsurance: true, contactPerson: 'איש קשר לבדיקה', phone: '052-1234567', email: `c-${crypto.randomUUID().slice(0, 6)}@example.test` },
