...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json().catch(() => null)) as { operationId?: unknown; recipient?: Record<string, unknown>; templateId?: unknown; channel?: unknown } | null
    ...
    if (typeof body.operationId !== 'string' || !/^[A-Za-z0-9:_-]{8,120}$/.test(body.operationId)) return NextResponse.json({ error: { message: 'חסר מזהה פעולה.' } }, { status: 400 })
    const result = await quickSend(session, { operationId: body.operationId, recipient, templateId: body.templateId, channel, ip: clientIp(request), userAgent: request.headers.get('user-agent') })
    if (!result.ok) return NextResponse.json({ error: { message: result.message }, state: result.state ?? null }, { status: result.state === 'in_progress' ? 409 : 400 })
