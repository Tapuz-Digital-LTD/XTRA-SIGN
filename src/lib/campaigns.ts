...
export type CampaignTab = 'overview' | 'joining' | 'audience' | 'agreements' | 'setup' | 'distributions' | 'reports' | 'settings'
...
export function tabsFor(entry: EntryMethod, options: { setup?: boolean } = {}): CampaignTab[] {
  void entry
  const tabs: CampaignTab[] = ['overview', 'joining', 'audience', 'agreements', 'distributions', 'reports', 'settings']
  if (options.setup) tabs.splice(tabs.indexOf('agreements') + 1, 0, 'setup')
  return tabs
}
...
export const TAB_INTROS: Record<CampaignTab, string> = {
  overview: 'מה דורש את תשומת הלב שלכם: המספרים המרכזיים וכרטיסי העבודה שמובילים למקום הנכון.',
...
export const LEGACY_TABS: Record<string, CampaignTab> = { suppliers: 'audience', leads: 'joining', registrations: 'joining', invitations: 'joining', tracking: 'joining' }
